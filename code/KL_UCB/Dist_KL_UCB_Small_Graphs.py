...
class Dist_KL_UCB:
    ...
    def __init__(self, T, arm_distributions, G, sigma):
        ...
        if type(sigma) == list:
            ...
            self.sigma = [sigma for i in range(N)]
        self.regrets = None
    ...
    def Q(self, t, Ni, agent):
        if (t > 1): return 3*(1+self.sigma[agent])*(np.log(t+1) + 3*np.log(np.log(t+1)))/(2*Ni)
        return 0
            ...
            for agent in range(self.N):
                ...
                for k in range(self.M):
                    if n[agent, k, t-1] <= m[agent, k, t-1] - self.M:
                        A.add(k)
                if len(A) == 0:
                    ucbs = [self.newton(n[agent, arm, t-1], z[agent, arm, t-1], self.Q(t-1, num_neighbors[agent], agent)) for arm in range(self.M)]
                    a = np.argmax(ucbs)
                else:
