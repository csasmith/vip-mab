...
class Dist_UCB:
    ''' Representation of a multi-agent bandit problem and a method to run the decentralized UCB algorithm on this problem
        ...
        T: The number of time steps the Dist_UCB algorithm will run for.
    ...
    def C(self, t, n):
        return np.sqrt((4*np.log(t+1))/(3*n))

    def plot_regret(self):
        ''' Plots regret of best and worst agent from last run

            Note: make sure Dist_UCB.run() was called before calling this method
        '''
    ...
    def run(self):
        ''' Run Dist_UCB on the bandit problem held by self 
            ...
            for agent in range(self.N):
                ...
                for k in range(self.M):
                    if n[agent, k, t-1] <= m[agent, k, t-1] - self.M:
                        A.add(k)
                if len(A) == 0:
                    ucbs = [z[agent, arm, t-1] + self.C(t-1, n[agent, arm, t-1]) for arm in range(self.M)]
                    a = np.argmax(ucbs)
                else:
...
#distucb = Dist_UCB(T, distributions)
#distucb.run()
#distucb.plot_regret()
